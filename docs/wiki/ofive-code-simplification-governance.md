...
| `node scripts/check-event-subscription-guards.mjs` | 阻止 UI 组件绕过语义 bus、store 或 plugin hub 直接订阅后端事件/stream。 |
...
- `scripts/check-event-subscription-guards.mjs`
- `scripts/check-store-state-tests.mjs`
