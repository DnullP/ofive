...
| `../obeditor/src/components/MarkdownReadView.tsx` | `MarkdownReadView`, `ReadModeFrontmatterPanel`, `ReadModeImageEmbed`, `ReadModeLatex`, `ReadModeMermaidDiagram`, `ReadModeWikiLinkAnchor` | obeditor read view | local refs/state, injected host capabilities | Read view consumes canonical Markdown content from the editor tab. Image/diagram render cache may stay local. WikiLink open/preview and media reads are injected through `createOfiveEditorCapabilities`. | obeditor read-mode tests plus ofive read-mode E2E. |
...
| `../obeditor/src/plugins/components/FrontmatterYamlVisualEditor.tsx` | `FrontmatterYamlVisualEditor`, `FrontmatterInlineTextField` | obeditor subcomponent | local refs/state, injected host capabilities | Visual editor edits local row drafts, then commits to the CodeMirror document owner. It must not persist directly or fork Markdown content. | obeditor frontmatter tests plus ofive editor E2E. |
| `../obeditor/src/plugins/components/MarkdownTableVisualEditor.tsx` | `MarkdownTableVisualEditor` | obeditor subcomponent | local refs/state, injected host capabilities | Table visual state is a projection of the CodeMirror document; commits go back through editor transactions. Local selection/focus only. | obeditor table tests plus ofive editor E2E. |
| `../obeditor/src/plugins/components/MarkdownTableCellLatex.tsx` | `TableCellLatex` | obeditor renderer | props only | Keep pure renderer. | obeditor table latex render tests if changed. |
| `../obeditor/src/plugins/syntaxPlugins/wikiLinkPreviewExtension.tsx` | `WikiLinkPreviewCard` | obeditor popover | injected `wikiLinks` capability | Preview data is request-local with cancellation; ofive provides vault/project-reader resolution through `createOfiveEditorCapabilities`. | obeditor wikilink preview tests plus ofive capability tests. |
