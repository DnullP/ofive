...
import {
    clearCodeSelectionIfInside,
    getElementFromNode,
    normalizeSelectionDisplayText,
    resolveProjectReaderCodeSelection,
    resolveSelectionBoundaryLocation,
    selectDomTextElementContents,
    selectProjectReaderCodeRange,
    type ProjectReaderCodeSelectionPayload,
} from "./projectReaderCodeSelection";
...
export function ProjectReaderCodeTab(
    ...
    const rememberCurrentCodeSelection = (scroller: HTMLElement): ProjectReaderCodeSelectionPayload | null => {
    ...
    useEffect(() => {
        const handleSelectionChange = (): void => {
            ...
            rememberCurrentCodeSelection(scroller);
        };
    ...
    const handleCodeMouseDown = (event: MouseEvent<HTMLElement>): void => {
        if (event.button === 2) {
            const payload = rememberCurrentCodeSelection(event.currentTarget)
                ?? pendingContextSelectionRef.current;
            if (payload && selectProjectReaderCodeRange(event.currentTarget, payload.range)) {
                pendingContextSelectionRef.current = payload;
                event.preventDefault();
                event.stopPropagation();
            }
    ...
    const handleCodeContextMenu = async (event: MouseEvent<HTMLElement>): Promise<void> => {
        ...
        const payload = rememberCurrentCodeSelection(scroller) ?? pendingContextSelectionRef.current;
        if (!payload) {
            event.preventDefault();
            event.stopPropagation();
            return;
        ...
        if (!selectProjectReaderCodeRange(scroller, payload.range)) {
            pendingContextSelectionRef.current = null;
            event.preventDefault();
            event.stopPropagation();
            return;
