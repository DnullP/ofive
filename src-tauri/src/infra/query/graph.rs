//! # Markdown 图谱查询基础设施模块
//!
//! 负责从索引中读取 Markdown 节点与链接边，不直接依赖 Tauri `State`。

use crate::infra::query::query_index;
use crate::shared::vault_contracts::VaultMarkdownGraphResponse;
use std::path::Path;

/// 在指定仓库根目录下读取 Markdown 图谱。
pub(crate) fn get_current_vault_markdown_graph_in_root(
    vault_root: &Path,
