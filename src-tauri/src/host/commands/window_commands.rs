...
//! 并同步更新当前主窗口的 Windows Acrylic 配置。

use crate::host::window_effects::{
    self, WindowsAcrylicEffectConfig,
};
use crate::state::AppState;
use std::time::Instant;
use tauri::{State, WebviewWindow};

/// 更新主窗口 Windows Acrylic 原生效果参数。
///
/// - `config`：前端下发的 Acrylic 参数快照
/// - `window`：当前调用窗口
/// - `state`：后端共享运行时状态
/// - 返回：成功或错误信息
/// - 副作用：更新内存中的 Acrylic 配置，并立即重设当前窗口原生效果
/// - 并发：短暂持有状态锁，不跨 await
#[tauri::command]
pub fn update_main_window_acrylic_effect(
    ...
    window_effects::apply_windows_effect_config(&window, &config, is_focused)
        .map_err(|error| format!("apply windows acrylic config failed: {error}"))?;
