//! # AI sidecar 运行时模块
//!
//! 负责 sidecar 生命周期、健康检查与 gRPC 客户端连接。
...
use super::{pb, SIDECAR_HEALTH_RETRY_COUNT, SIDECAR_HEALTH_RETRY_DELAY_MS};
...
pub(crate) async fn ensure_sidecar_endpoint(
...
async fn wait_for_sidecar_ready(endpoint: String) -> Result<(), String> {
    ...
    for _ in 0..SIDECAR_HEALTH_RETRY_COUNT {
        match connect_client(endpoint.clone()).await {
