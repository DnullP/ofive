...
//!
//! ## 依赖
//! - `query_index` — SQLite 索引查询
//! - `state` — 全局仓库状态
//! - `types::BacklinkItem` — 返回类型
//!
//! ## 导出
//! - `get_backlinks_for_file` — 带 `State` 的 Tauri 命令包装
//! - `get_backlinks_for_file_in_root` — 直接指定 vault root 的辅助函数（用于测试）

use crate::state::{get_vault_root, AppState};
use crate::vault_commands::query_index;
use crate::vault_commands::types::BacklinkItem;
use std::path::Path;
use tauri::State;

/// 查询指定文件的反向链接（直接指定 vault_root，无需 State）。
///
/// # 参数
/// - `vault_root` — 仓库根目录
...
/// 查询指定文件的反向链接（通过全局 State 获取 vault_root）。
///
/// # 参数
/// - `relative_path` — 目标文件相对路径
/// - `state` — 应用共享状态
...
pub fn get_backlinks_for_file(
    ...
    get_backlinks_for_file_in_root(&vault_root, &relative_path)
}
