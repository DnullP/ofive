//! # Markdown AST 模块
//!
//! 提供纯后端 Markdown AST 解析能力：
//! - 输入原始 Markdown 文本
//! - 输出可序列化、可通过 Tauri 返回给前端的 AST 树
//!
//! ## 设计说明
//!
//! 本模块基于 `pulldown-cmark` 的事件流构建轻量 AST，
//! 不直接暴露第三方库内部类型，避免前后端协议与外部库耦合。
//!
//! ## 导出
//!
//! - [`parse_markdown_to_ast`]：将 Markdown 文本解析为 AST 根节点
//! - [`get_vault_markdown_ast_in_root`]：读取指定笔记并返回 AST
//! - [`get_vault_markdown_ast`]：Tauri 命令包装

use crate::state::{get_vault_root, AppState};
use crate::vault_commands::fs_helpers::resolve_markdown_path;
use crate::vault_commands::types::{MarkdownAstNode, ReadMarkdownAstResponse};
...
/// # 异常情况
/// - 当事件流出现不平衡嵌套时返回错误字符串
pub fn parse_markdown_to_ast(content: &str) -> Result<MarkdownAstNode, String> {
...
/// 读取指定 Markdown 笔记并返回 AST。
///
/// # 参数
/// - `vault_root`：当前仓库根目录
/// - `relative_path`：目标笔记相对路径
///
/// # 返回
/// - 成功时返回文件相对路径与 AST
...
/// Tauri 命令包装：读取当前仓库笔记 AST。
pub fn get_vault_markdown_ast(
    relative_path: String,
    state: State<'_, AppState>,
) -> Result<ReadMarkdownAstResponse, String> {
    let vault_root = get_vault_root(&state)?;
    get_vault_markdown_ast_in_root(&vault_root, relative_path)
}
