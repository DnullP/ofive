...
function readDependencySpec(packageJson, dependencyName) {
...
function resolveLocalSiblingDependencies(packageJson) {
    return [
        ["layout-v2", readDependencySpec(packageJson, "layout-v2")],
        ["obeditor", readDependencySpec(packageJson, "obeditor")],
    ]
        .map(([name, spec]) => [name, resolveLocalDependencyPath(spec)])
        .filter((entry) => Boolean(entry[1]));
}

function main() {
    const packageJson = readPackageJson(packageJsonPath);
    const localDependencies = resolveLocalSiblingDependencies(packageJson);

    for (const [dependencyName, dependencyRoot] of localDependencies) {
        const dependencyPackageJsonPath = path.join(dependencyRoot, "package.json");
        if (!existsSync(dependencyPackageJsonPath)) {
            throw new Error(`[push] local ${dependencyName} dependency is missing: ${dependencyPackageJsonPath}`);
        }

        ensureCleanWorktree(dependencyRoot, dependencyName);
        pushBranch(dependencyRoot, dependencyName);
    }
